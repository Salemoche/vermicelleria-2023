const ImageItemComponent = ({sectionItem: {image, mask, aspectRatio, width, extraSpace, hideOnScreens}}) => {
    ...
    return (
        <div className={`only:col-start-${ aspectRatio === 'twoThree' && width == 6 ? '4' : ''} col-span-${ aspectRatio === 'twoThree' && width == 6 ? 6 : 12} lg:col-span-${width} ${aspectRatioClass} mb-${ extraSpace ? '8' : '2' } lg:mb-${ extraSpace ? '20' : '4' } ${ hideOnScreens.includes('mobile') ? 'hidden' : 'inline-block' } ${ hideOnScreens.includes('desktop') ? 'lg:hidden' : 'lg:inline-block' }`}>
            { image[0] && <img 
